...
import ROOM_TYPE_FIELD from "@salesforce/schema/MV_Room__c.MV_SEL_Type__c";
import ROOM_OBJECT from "@salesforce/schema/MV_Room__c";
...
export default class HotelFilterAppHome extends LightningElement {
  ...
  countryListfromApex({ data, error }) {
    if (data) {
      data.forEach((element) => {
        this.countryList = [
          ...this.countryList,
          { label: element.MV_SEL_Country__c, value: element.MV_SEL_Country__c }
        ];
      });
    }
    if (error) {
  ...
  getCityList() {
    cityList({ countryName: this.countrySelectedValue })
      .then((result) => {
        this.cityList = [];
        result.forEach((element) => {
          this.cityList = [
            ...this.cityList,
            { label: element.MV_TXT_City__c, value: element.MV_TXT_City__c }
          ];
        });
      })
      .catch((error) => {
  ...
  getHotelsList() {
    hotelList({
      countryName: this.countrySelectedValue,
      cityName: this.citySelectedValue
    })
      .then((result) => {
        this.hotelList = [];
        ...
        result.forEach((element) => {
          this.hotelList = [
            ...this.hotelList,
            { label: element.Name, value: element.Name, Id: element.Id }
          ];
        });
      })
      .catch((error) => {
  ...
  handleTypeChange(event) {
    ...
    if (event.target.checked) {
      ...
      this.filters.rooms = this.filters.rooms.filter(
